...
class CodeModel(BasicModel):
    ...
    @classmethod
    def get_source(cls, target, code_type, version=None):
        if version is not None:
            return cls.query(cls.code_type == code_type, cls.target == target.key, cls.version == int(version)).order(-cls.sort).get()
        else:
            return cls.query(cls.code_type == code_type, cls.target == target.key).order(-cls.version).get()
